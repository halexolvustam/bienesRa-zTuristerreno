...
export function Contact() {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', development: '', contactMethod: '', message: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (form.contactMethod === 'Email' || form.contactMethod === '') {
      emailjs.send('service_mqsgmg9', 'template_y4phi5c', templateParams, '1ge-ilMlK-aYoiihp')
        .then(() => { setIsSubmitting(false); setIsSubmitted(true); })
        .catch(() => { setIsSubmitting(false); alert(isEn ? 'Error sending email. Try WhatsApp.' : 'Error al enviar. Intenta por WhatsApp.'); });
    } else {
      const msg = encodeURIComponent(`Hola, mensaje desde el sitio web:\n\nNombre: ${form.name}\nEmail: ${form.email}\nTelefono: ${form.phone || 'No proporcionado'}\nDesarrollo: ${devName}\n\nMensaje:\n${form.message}`);
    ...
    <section id="contact" className="py-20 bg-stone-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <h2 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-3">{isEn ? "Get Your Free Quote" : "Cotiza Gratis"}</h2>
          <p className="text-lg text-gray-500">{isEn ? "No commitment. We'll get back to you right away." : "Sin compromiso. Te respondemos de inmediato."}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="bg-green-900 text-white rounded-2xl p-8 space-y-4">
            <div>
              <h3 className="text-xl font-bold mb-1">{isEn ? "Contact Information" : "Información de Contacto"}</h3>
              <p className="text-green-300 text-sm">{isEn ? "Free quote, no commitment." : "Cotización gratuita, sin compromiso."}</p>
            </div>

            <a href="tel:+525566545971" className="flex items-center gap-4 w-full bg-green-800 hover:bg-green-700 transition-colors rounded-xl px-4 py-3">
              <div className="w-10 h-10 bg-green-700 rounded-full flex items-center justify-center flex-shrink-0"><Phone className="w-5 h-5" /></div>
              <div>
            ...
            <a href="https://wa.me/525566545971?text=Hola%2C%20quiero%20cotizar%20gratis%20un%20terreno" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 w-full bg-[#25D366] hover:bg-[#1ebe5d] transition-colors rounded-xl px-4 py-3">
              <div className="w-10 h-10 bg-[#1ebe5d] rounded-full flex items-center justify-center flex-shrink-0">{waIcon}</div>
              <div>
            ...
            <div className="flex items-start gap-4 px-1 pt-2">
              <div className="w-10 h-10 bg-green-800 rounded-full flex items-center justify-center flex-shrink-0"><MapPin className="w-5 h-5" /></div>
              <div>
                <p className="text-green-300 text-sm">{t.contact.info.address}</p>
                <p className="font-semibold text-sm">{isEn ? "Merida, Yucatan, Mexico" : "Mérida, Yucatán, México"}</p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            {isSubmitted ? (
              <div className="text-center py-20">
                ...
                <p className="text-gray-600 mb-6">{isEn ? "I will get back to you soon!" : "¡Te respondo pronto!"}</p>
                <Button className="bg-green-900 hover:bg-green-800" onClick={() => { setIsSubmitted(false); setForm({ name: '', email: '', phone: '', development: '', contactMethod: '', message: '' }); }}>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{isEn ? "Development of Interest" : "Desarrollo de Interés"}</label>
                    <select id="development" value={form.development} onChange={handleChange} className={inputClass}>
                      ...
                      {activeDevelopments.map(dev => (<option key={dev.id} value={dev.name}>{dev.name}</option>))}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{isEn ? "Would you like to receive info by email or WhatsApp?" : "¿Desea recibir una cotización a su correo o WhatsApp?"}</label>
                ...
                <Button type="submit" className="bg-green-900 hover:bg-green-800 text-white px-8 py-3 rounded-lg" disabled={isSubmitting}>
                  {isSubmitting ? (<><span className="animate-spin mr-2">⏳</span>{t.contact.form.sending}</>) : (<><Send className="w-4 h-4 mr-2" />{isEn ? "Get free quote" : "Cotizar gratis"}</>)}
                </Button>
              </form>
            )}
          </div>
